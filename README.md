...
### Validation
- Because of the issues with trying to vailate code that uses jinja templating pages were tested  URI by direct input of source code  into [validator.w3.org](https://validator.w3.org/). There were residual errors resulting from the use of code from django's crispy forms. The results of this can be seen  [here](/media/htmlvalidation.png). 
...
- Throughout development Python code was tested with [flake 8](https://flake8.pycqa.org/en/latest/) and problems identified were corrected where appropriate. [PEP8 online](http://pep8online.com/) was used to help formatting. Some lines that did not comply with PEP8 due to being too long were left - when they could not be made PEP8 compliant. These were mainly in settings.py and migrations but tehre were seom lines that, when corrected in PEP 8 online then no longer worked in the code. These occured in checkout's webhook_handler.pyand webhooks.py,  and in widgets.py in gins and distilleries.
